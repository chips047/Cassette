...
class PlaybackManager(QObject):
    ...
    def _setup_parameters(self):
        self.win_s = 512
        self.hop_s = 256 if not sys.platform == "linux" else 512
    ...
    def _open_stream(self):
        ...
        self.beat_thread = threading.Thread(
            target = self._beat_emitter_worker,
            daemon = True
        )
    ...
    def audio_callback(self, outdata, frames, time_info, status):
        ...
        self._process_beat_detection(block)
    ...
    def _process_beat_detection(self, block):
        # Быстро сводим в моно
        mono_block = np.mean(block, axis=1).astype(np.float32)

        # aubio.onset работает быстро благодаря C-реализации
        if self.onset_detector(mono_block):
            current_time = time.time()
            
            # Считаем RMS ТОЛЬКО в момент удара (экономим CPU)
            rms = np.float32(np.sqrt(np.mean(mono_block**2)))
            
            is_heavy = False
            if rms > self.heavy_rms_threshold and (current_time - self.last_heavy_time) > self.heavy_cooldown:
                is_heavy = True
                self.last_heavy_time = current_time

            # Мгновенно кидаем в очередь без блокировок. 
            # Если очередь переполнена (UI завис), мы просто игнорируем бит, чтобы не сломать звук.
            try:
    ...
    def tape(
        ...
        self.set_speed(end_speed if end_speed is not None else self.speed, steps, duration, stop_on_finish)
